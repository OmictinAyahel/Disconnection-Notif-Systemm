$(document).ready(function() {
    ...
    $('#fileInput').on('change', function(e) {
        ...
        reader.onload = function(e) {
            ...
            var headers = jsonData.shift();
            formattedData = jsonData.map(function(row) {
                var obj = {};
                headers.forEach(function(header, i) {
                    var cellValue = row[i];
                    if (header === 'Unpaid Bills') {
    ...
    function showSendSMSButtons(showSingle, showMultiple) {
        ...
        if(showSingle || showMultiple) {
            $('#sendSMSModal').modal('show');
            $('.wrapper-body').css('z-index', '1051');
            if (showSingle) {
        ...
        if (button.text() === 'View History') {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    button.text('');
                    button.addClass('fa-solid fa-arrow-right');
                    var data = JSON.parse(response);

                    if (historyTable) {
                        // Clear the existing DataTable and reload data
                    ...
                    exportButton.css('margin-left', '10px').appendTo(lengthControl);
            ...
            button.removeClass('fa-solid fa-arrow-right');
    ...
    function renderDataTable(data) {
        ...
        if (lengthControl) {
            // lengthControl.empty();                         
            // uploadLink.detach();
            // var firstElement = lengthControl.find('label:first-child');
            // firstElement.css('float', 'right'); // Move to right
            // uploadLink.detach().css('margin-left', '10px').insertAfter(firstElement);

            var firstElement = lengthControl.find('label:first-child');
            firstElement.contents().filter(function() {
                return this.nodeType === 3; // Remove only text nodes
            }).remove();
            firstElement.css('float', 'right'); // Move to right
            uploadLink.detach().insertAfter(firstElement);
        }
        
        uploadLink.css('margin-left', '10px').appendTo(lengthControl);             
